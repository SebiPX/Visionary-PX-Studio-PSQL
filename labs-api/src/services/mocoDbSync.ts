      ...
      if (Array.isArray(p.tasks)) {
        for (const task of p.tasks) {
          ...
          if (taskRes.rows.length === 0) {
            ...
            `, [projectId, task.name, task.description || '', mocoStatus, rate, estHours, task.billable, task.id]);
          } else {
            await pool.query(`
              UPDATE agency_tasks 
            ...
            `, [task.name, task.description || '', mocoStatus, rate, estHours, task.billable, task.id, projectId]);
          }
        }
      }
    }
