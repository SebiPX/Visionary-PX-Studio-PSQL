...
Bitte extrahiere folgende Daten im JSON Format:
{
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div>
        <h3 className="text-xl font-bold text-foreground mb-6">AI Insights & Mustererkennung</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {posts.map(post => {
              ...
              return (
                 <div key={post.id} className="glass-card rounded-xl border border-border/50 overflow-hidden flex flex-col">
                    <div className="h-48 bg-black relative">
                       ...
                       <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                          <p className="text-foreground text-xs font-bold line-clamp-1">{post.caption}</p>
                       </div>
                    </div>
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                       {hasAnalysis ? (
                          <div className="space-y-3 flex-1">
                             ...
                             <div className="flex flex-wrap gap-1">
                                {post.detected_patterns?.map((tag: string, i: number) => (
                                   <span key={i} className="px-2 py-0.5 rounded-full border border-border text-[10px] text-foreground/90">
                                      #{tag}
                                   </span>
                                ))}
                             </div>
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center py-4 opacity-50 text-center">
                             <span className="material-icons-round mb-2 text-muted-foreground">model_training</span>
                             <p className="text-xs text-muted-foreground">Noch keine Analyse vorhanden</p>
                          </div>
                       )}
                       ...
                       <button 
                          ...
                          className="mt-4 w-full py-2 rounded-lg bg-white/5 hover:bg-white/10 text-foreground text-xs font-medium border border-border transition-colors flex items-center justify-center gap-2"
                       >
