  ...
  return (
    <div className="max-w-6xl mx-auto w-full p-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold font-display tracking-tight text-foreground mb-2">My Event Projects</h2>
      ...
      {isCreating && (
        <div className="bg-card rounded-2xl border border-border p-6 mb-8 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs text-muted-foreground uppercase tracking-wider mb-2 font-semibold">Guests</label>
              <input 
                type="number" 
                value={newGuestCount} 
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button 
              ...
              className="px-6 py-2 rounded-lg bg-brand-600 hover:bg-brand-500 text-foreground font-medium disabled:opacity-50 transition-colors flex items-center gap-2"
            >
      ...
      {loading && projects.length === 0 ? (
        ...
        <div className="text-center py-20 border-2 border-dashed border-border/50 rounded-2xl bg-white/5">
          ...
          <p className="text-muted-foreground mb-6 max-w-sm mx-auto">Start a new event project to generate out-of-the-box matrices and SCAMPER refinements.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="space-y-2 mb-6 text-sm text-muted-foreground">
                <div className="flex justify-between">
                  <span>Occasion:</span>
                  <span className="text-foreground font-medium text-right truncate pl-4 max-w-[60%]">{p.occasion}</span>
                </div>
                <div className="flex justify-between">
                  <span>Guests:</span>
                  <span className="text-foreground font-medium">{p.guest_count}</span>
                </div>
                <div className="flex justify-between">
                  <span>Last Updated:</span>
                  <span className="text-foreground font-medium">{new Date(p.updated_at).toLocaleDateString()}</span>
                </div>
              </div>
