...
export const ScamperStep: React.FC = () => {
  ...
  if (!concepts || concepts.length === 0) {
    return <div className="p-8 text-center text-muted-foreground">Waiting for Concepts data... Please hold on.</div>;
  }

  return (
    <div className="max-w-7xl mx-auto w-full pb-20">
      <div className="mb-8">
        <h2 className="text-2xl font-bold font-display tracking-tight text-foreground mb-2">Step 3: SCAMPER Refinements & Evaluation</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {concepts.map((concept, idx) => {
          ...
          return (
            <div key={concept.id} className="bg-card rounded-2xl border border-border p-6 shadow-xl flex flex-col h-full relative overflow-hidden group hover:border-brand-500/30 transition-all">
              
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-border/50">
                <h3 className="text-lg font-bold text-foreground">Concept {idx + 1}</h3>
              ...
              <div className="flex-grow space-y-6">
                ...
                {/* Selected Parameters */}
                <div>
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Core Parameters</h4>
                  <div className="flex flex-wrap gap-2">
                    {Object.values(concept.selected_parameters).map((val: any) => (
                      <span key={val} className="px-2 py-1 bg-white/5 border border-border rounded text-xs text-foreground/90">
                        {val}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground/90 leading-relaxed italic bg-brand-500/5 p-3 rounded-lg border border-brand-500/10 mb-4">
                  ...
                  {concept.scamper_refinements?.real_world_validation && (
                    <div className="bg-muted/50 p-3 rounded-lg border border-border/50 space-y-3">
                      <h4 className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground uppercase tracking-wider border-b border-border/50 pb-2 mb-2">
                      ...
                      {concept.scamper_refinements.real_world_validation.location && (
                        <div>
                          <span className="block text-[10px] uppercase text-muted-foreground mb-1">Real Location</span>
                          <strong className="text-green-400 text-sm block">{concept.scamper_refinements.real_world_validation.location.name}</strong>
                          <p className="text-xs text-muted-foreground mt-1 mb-2">{concept.scamper_refinements.real_world_validation.location.description}</p>
                          {concept.scamper_refinements.real_world_validation.location.address && <p className="text-[11px] text-foreground/90 mt-1"><span className="opacity-50">📍</span> {concept.scamper_refinements.real_world_validation.location.address}</p>}
                          {concept.scamper_refinements.real_world_validation.location.contact && <p className="text-[11px] text-foreground/90"><span className="opacity-50">📞</span> {concept.scamper_refinements.real_world_validation.location.contact}</p>}
                          {concept.scamper_refinements.real_world_validation.location.website && <p className="text-[11px] text-blue-400 truncate"><a href={concept.scamper_refinements.real_world_validation.location.website.startsWith('http') ? concept.scamper_refinements.real_world_validation.location.website : `https://${concept.scamper_refinements.real_world_validation.location.website}`} target="_blank" rel="noreferrer" className="hover:underline"><span className="opacity-70">🌐</span> {concept.scamper_refinements.real_world_validation.location.website}</a></p>}
                        </div>
                      )}
                      ...
                      {concept.scamper_refinements.real_world_validation.nearby_vendors && concept.scamper_refinements.real_world_validation.nearby_vendors.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-border/50">
                          <span className="block text-[10px] uppercase text-muted-foreground mb-2">Nearby Vendors</span>
                          <div className="space-y-3">
                            {concept.scamper_refinements.real_world_validation.nearby_vendors.map((v: any, vi: number) => (
                              <div key={vi} className="bg-white/5 p-2 rounded border border-border/50">
                                <strong className="text-blue-400 text-xs block">{v.name} <span className="text-[10px] text-muted-foreground font-normal">({v.type})</span></strong>
                                <p className="text-[10px] text-muted-foreground mt-1">{v.description}</p>
                                <p className="text-[10px] text-foreground/90 mt-1">📞 {renderWithLinks(v.contact)}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                {/* Evaluation */}
                <div className="flex gap-4 p-3 bg-black/20 rounded-lg border border-border/50">
                  <div className="flex-1">
                    <span className="block text-[10px] uppercase text-muted-foreground mb-1">HNW Score</span>
                  ...
                  <div className="flex-1 border-l border-border pl-4">
                    <span className="block text-[10px] uppercase text-muted-foreground mb-1">Budget</span>
              ...
              <button 
                ...
                className="mt-6 w-full py-3 rounded-xl bg-white/5 hover:bg-brand-600 border border-border hover:border-transparent text-foreground font-bold transition-all flex items-center justify-center gap-2 group-hover:shadow-[0_0_20px_rgba(234,88,12,0.3)] disabled:opacity-50"
              >
