  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-2">
        <button type="button" onClick={onCancel} className="px-4 py-2 text-muted-foreground hover:text-foreground text-sm transition-colors">
          Abbrechen
        </button>
        <button
          type="submit"
    ...
    <div className="group flex items-start gap-4 p-4 bg-card/60 border border-border rounded-2xl hover:border-border/80 hover:bg-card/80 transition-all">
      ...
      <div className="w-10 h-10 shrink-0 rounded-xl bg-muted border border-border/80 flex items-center justify-center overflow-hidden">
        {favicon && !imgError ? (
          ...
          <span className={`w-full h-full flex items-center justify-center text-sm font-bold text-foreground rounded-xl ${avatarColor(link.titel)}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {link.beschreibung && (
          <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{link.beschreibung}</p>
        )}
      ...
      {isAdmin && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-muted-foreground hover:text-red-400 hover:bg-red-900/30 transition-colors"
            title="Löschen"
          >
            <Trash2 size={14} />
          </button>
