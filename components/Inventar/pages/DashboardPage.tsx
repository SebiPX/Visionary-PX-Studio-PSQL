...
function StatPill({ label, value, alert }: { label: string; value: number | string; alert?: boolean }) {
  return (
    <div className={`flex items-center gap-3 bg-card/60 border rounded-xl px-4 py-3 ${alert ? 'border-red-500/40' : 'border-border'}`}>
      <span className={`text-2xl font-bold ${alert ? 'text-red-300' : 'text-foreground'}`}>{value}</span>
    ...
    >
      <div className="w-8 h-8 shrink-0 rounded-lg bg-muted border border-input flex items-center justify-center overflow-hidden">
        {favicon && !imgError ? (
          ...
          <span className={`w-full h-full flex items-center justify-center text-xs font-bold text-foreground rounded-lg ${avatarColor(link.titel)}`}>
...
function PinnedLoginCard({ login }: { login: Login }) {
  return (
    <div className="flex items-center gap-3 bg-card/50 border border-border rounded-xl p-3">
      <div className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center text-xs font-bold text-foreground ${avatarColor(login.name || login.id)}`}>
        {(login.name || login.website || '?').charAt(0).toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-foreground truncate">{login.name || login.website}</p>
      ...
      <div className="p-6 space-y-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Willkommen im PX INTERN</p>
          </div>
          <button
            onClick={() => setDrawerOpen(true)}
            className="flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground hover:text-foreground bg-card hover:bg-muted border border-border rounded-xl transition-all"
          >
            <Settings size={14} />
            Anpassen
          </button>
